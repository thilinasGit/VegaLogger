...
int fMag[3] ;
int imuSlowerCounter=0,imuSlowVal=50;

///// Jason obj for mqtt
DynamicJsonDocument jsonDoc = DynamicJsonDocument(680);  
...
void setup() {
  ...
  lcdUpdate(" ETX  Telemetry ", 1, 1);
  lcdUpdate("  Logger v2.7   ", 2, 0);
  delay(4500);
  ...
  lcdUpdate("Connected", 1, 1); ////////////////////////////////////////////////not required
...
void mainMenuPg() {                         ///0                  ////////////////////////////////////////////MMENNUUUUUUUUUUUUUUUUUUU
  ...
  switch (getBtnStatus()) {
    ...
    case 200:
        lcdUpdate("About Device", 0, 1);
        lcdUpdate("  Developed by", 1, 1);
        lcdUpdate("   B. Thilina", 2, 1);
        lcdUpdate(" Firmware v2.6", 3, 0);
        delay(2500);
        break;
...
void jsonStuff() {
  ...
  if (commandCycle == 2) {
    ...
    jsonDoc["ts"] = String(getEpochTime());
    if (dataSaverEnabled) {      
      if (getEpochTime() % 2 == 0){
...
static void SensorDataUpdata(uint32_t uiReg, uint32_t uiRegNum)
{
  int i;
  imuSlowerCounter--;
  if(imuSlowerCounter<0) {
  imuSlowerCounter=imuSlowVal;
